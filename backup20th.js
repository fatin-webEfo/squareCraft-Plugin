(async function squareCraft() {
    ...
    function createWidget() {
      ...
      widgetContainer.classList.add("squareCraft-fixed","squareCraft-hidden", "squareCraft-text-color-white", "squareCraft-universal", "squareCraft-z-99999");
    ...
    function createWidgetIcon() {
      ...
      widgetIcon.src = "https://i.ibb.co.com/pry1mVGD/Group-28-1.png";
      widgetIcon.classList.add("squareCraft-absolute", "squareCraft-hidden", "squareCraft-rounded-md", "squareCraft-w-12", "squareCraft-h-12", "squareCraft-bg-color-2c2c2c", "squareCraft-cursor-pointer", "squareCraft-z-9999");
  
      widgetIcon.style.display = "none"; // Hide on load
      ...
      widgetIcon.addEventListener("click", () => {
          ...
          // Ensure widget appears next to the icon
          if (widgetContainer.style.display === "block") {
              positionWidgetNextToIcon(widgetContainer, widgetIcon);
          }
      });
  ...
  // ✅ Position Widget Icon Logically (Left or Right)
  function positionWidgetIcon(targetElement) {
      const widgetIcon = document.getElementById("squarecraft-widget-icon");
      if (!widgetIcon || !targetElement) return;
      ...
      // Check if the target element is on the right side
      const isRightSide = rect.left + rect.width / 2 > windowWidth / 2;
  
      // Adjust icon position based on the element's location
      if (isRightSide) {
          iconLeft = window.scrollX + rect.left - widgetIcon.offsetWidth - 10; // Position left of the element
      } else {
          iconLeft = window.scrollX + rect.right + 10; // Position right of the element
      }
  
      iconTop = window.scrollY + rect.top + rect.height / 2 - widgetIcon.offsetHeight / 2;
  
      // Ensure widget icon stays within the viewport
      iconLeft = Math.max(10, Math.min(iconLeft, windowWidth - widgetIcon.offsetWidth - 10));
  
      // Apply new position
      widgetIcon.style.left = `${iconLeft}px`;
      widgetIcon.style.top = `${iconTop}px`;
      widgetIcon.style.display = "block"; // Ensure it's visible
  }
  
  // ✅ Position Widget Next to Icon
  function positionWidgetNextToIcon(widgetContainer, widgetIcon) {
      if (!widgetContainer || !widgetIcon) return;
  
      const iconRect = widgetIcon.getBoundingClientRect();
      const windowWidth = window.innerWidth;
      let widgetLeft;
  
      // If icon is near the right edge, place widget to its left
      if (iconRect.left + widgetContainer.offsetWidth > windowWidth) {
          widgetLeft = iconRect.left - widgetContainer.offsetWidth - 10;
      } else {
          widgetLeft = iconRect.right + 10;
      }
  
      widgetContainer.style.left = `${widgetLeft}px`;
      widgetContainer.style.top = `${iconRect.top}px`;
  }
  
  // ✅ Click Event Listener: Handles Showing/Hiding Widget and Icon
  document.body.addEventListener("click", (event) => {
      ...
      if (targetBlock) {
          positionWidgetIcon(targetBlock); // Show the widget icon logically
          selectedElement = targetBlock;
      } else if (!widgetContainer.contains(event.target) && !widgetIcon.contains(event.target)) {
          widgetContainer.style.display = "none"; // Hide widget on outside click
          widgetIcon.style.display = "none"; // Hide icon too
      }
  });
